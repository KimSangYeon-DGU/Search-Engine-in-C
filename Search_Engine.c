...
int search_table(char *word) {
	int sortedArray[9][2]; //[0]document 번호 ,[1] 단어 포함된 횟수  
	int k = 0;
	node_pointer s = Hash_Table;
	...
	while (s->link) { //다음 노드가 존재하면
		if (strcmp(modified_word, s->link->word) == 0) {
			...
			for (int document = 0; document < 9; document++) {
				...
				sortedArray[k][0] = document;
				sortedArray[k++][1] = word_number;
			}
			returntoCmp = cmp;
		}
		cmp++; //(비교값과 다른 상태)
	...
	for (int i = 1; i < 9; i++) //삽입 정렬하기위해. 
	{
		int next_1, next_2;
		int j;
		next_1 = sortedArray[i][0];
		next_2 = sortedArray[i][1];
		for (j = i - 1; j >= 0 && next_2 > sortedArray[j][1]; j--)
		{
			sortedArray[j + 1][0] = sortedArray[j][0];
			sortedArray[j + 1][1] = sortedArray[j][1];

		}
		sortedArray[j + 1][0] = next_1;
		sortedArray[j + 1][1] = next_2;
	}



	while (temp->link) { //다음 노드가 존재하면
		if (strcmp(modified_word, temp->link->word) == 0) {
			int s;
			for (s = 0; s < 9; s++)
			{
				int document = sortedArray[s][0];
				int word_number = sortedArray[s][1];
				if (word_number == 0)
					continue;
				printf("\n<document%d.txt> %s: %d\n", document + 1, word, word_number);
				for (int n = 1; n <= word_number; n++)
				{
					word_position = temp->link->info.position[document][n];
